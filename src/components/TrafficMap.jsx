...
import TomTomTrafficMap from './TomTomTrafficMap.jsx';
...
// Decorative markers keep the fallback useful when a TomTom API key is not configured.
const junctions = [
...
export default function TrafficMap({
  ...
  const footerMessage = isLiveMapEnabled
    ? 'TomTom traffic flow is active. Enter locations to calculate real driving ETA.'
    ...
    <section className={`overflow-hidden rounded-2xl border p-4 shadow-traffic backdrop-blur sm:p-5 ${panelClass}`}>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <span className="w-fit rounded-full border border-sky-300/40 bg-sky-400/10 px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-sky-200">
            {isLiveMapEnabled ? 'Live map' : 'Fallback map'}
          </span>
        </div>
      </div>
      ...
      <div className={`mj-map-grid relative min-h-[360px] overflow-hidden rounded-xl border shadow-inner sm:min-h-[480px] lg:min-h-[560px] ${mapBaseClass}`}>
        {isLiveMapEnabled ? (
          ...
          <>
            <motion.div
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: status.overlayOpacity, boxShadow: status.overlayGlow }}
            ...
            {vehicles.map((vehicle, index) => (
              <motion.div
                ...
                style={{ left: index % 2 === 0 ? '4%' : '72%', top: `${42 + index * 8}%` }}
        ...
        <div className={`absolute bottom-3 left-3 right-3 z-40 rounded-xl border p-3 shadow-lg backdrop-blur sm:bottom-4 sm:left-4 sm:right-4 ${labelClass}`}>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-semibold">{isLiveMapEnabled ? footerMessage : status.mapMessage}</p>
              <p className={`mt-1 text-xs ${mutedClass}`}>Monitoring Zone: Central City</p>
            </div>
            <div className="text-left sm:text-right">
              <p className="text-xs font-bold uppercase tracking-[0.16em]">
                {isLiveMapEnabled ? 'Live traffic layer' : status.estimatedDelay}
              </p>
